...
public class JCalculator extends JFrame
{
  ...
  // Mise à jour de l'interface apres une operation (jList et jStack)
  private void update()
  {
    // Modifier une zone de texte, JTextField.setText(string nom)
    // Modifier un composant liste, JList.setListData(Object[] tableau)
    if(calcState.error) {
      jNumber.setText("ERROR :3");
    } else {
      jNumber.setText(calcState.current);
    }
    if(!calcState.stack.empty()){
      Object[] objets = calcState.stack.array();
      String[] chaines = new String[objets.length];

      for (int i = 0; i < objets.length; i++) {
        chaines[i] = String.valueOf(objets[i]);
      }
        jStack.setListData(chaines);
  ...
  public JCalculator()
  {
    ...
    addOperatorButton("MR", 0, 1, Color.RED, new MemoryRecall(calcState));

    // Stockage d'une valeur en memoire
    addOperatorButton("MS", 1, 1, Color.RED, new MemoryStore(calcState));

    // Backspace
    addOperatorButton("<=", 2, 1, Color.RED, new Backspace(calcState));

    // Mise a zero de la valeur courante + suppression des erreurs
    addOperatorButton("CE", 3, 1, Color.RED, new ClearError(calcState));

    // Comme CE + vide la pile
    addOperatorButton("C",  4, 1, Color.RED, new Clear(calcState));

    // Boutons 1-9
    for (int i = 1; i < 10; i++) 
      addOperatorButton(String.valueOf(i), (i - 1) % 3, 4 - (i - 1) / 3, 
			Color.BLUE, new AddToCurrent(calcState, String.valueOf(i)));
    // Bouton 0
    addOperatorButton("0", 0, 5, Color.BLUE, new AddToCurrent(calcState, "0"));

    // Changement de signe de la valeur courante
    addOperatorButton("+/-", 1, 5, Color.BLUE, new Opposite(calcState));

    // Operateur point (chiffres apres la virgule ensuite)
    addOperatorButton(".", 2, 5, Color.BLUE, new AddToCurrent(calcState, "."));

    // Operateurs arithmetiques a deux operandes: /, *, -, +
    addOperatorButton("/", 3, 2, Color.RED, new Division(calcState));
    addOperatorButton("*", 3, 3, Color.RED, new Multiplication(calcState));
    addOperatorButton("-", 3, 4, Color.RED, new Subtraction(calcState));
    addOperatorButton("+", 3, 5, Color.RED, new Addition(calcState));

    // Operateurs arithmetiques a un operande: 1/x, x^2, Sqrt
    addOperatorButton("1/x", 4, 2, Color.RED, new Inverse(calcState));
    addOperatorButton("x^2", 4, 3, Color.RED, new Square(calcState));
    addOperatorButton("Sqrt", 4, 4, Color.RED, new SquareRoot(calcState));

    // Entree: met la valeur courante sur le sommet de la pile
    addOperatorButton("Ent", 4, 5, Color.RED, new AddCurrToStack(calcState));
